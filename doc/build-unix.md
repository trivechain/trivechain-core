UNIX BUILD NOTES
====================
Some notes on how to build Trivechain in Unix.

(for OpenBSD specific instructions, see [build-openbsd.md](build-openbsd.md))

Base build dependencies
-----------------------
Building the dependencies and Trivechain requires some essential build tools and libraries to be installed before.
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Trivechain should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, Trivechain may be compiled in
...
Add `CC=egcc CXX=eg++ CPP=ecpp` to the dependencies build and the Trivechain build:
```bash
$ cd depends
