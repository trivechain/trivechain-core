...
The OS X configuration assumes trivechain will be set up for the current user.

2. Configuration
---------------------------------

At a bare minimum, trivechain requires that the rpcpassword setting be set
when running as a daemon.  If the configuration file does not exist or this
setting is not set, trivechain will shutdown promptly after startup.

This password does not have to be remembered or typed as it is mostly used
as a fixed token that trivechain and client programs read from the configuration
file, however it is recommended that a strong and secure password be used
as this password is security critical to securing the wallet should the
wallet be enabled.

If trivechain is run with the "-server" flag (set by default), and no rpcpassword is set,
...
This allows for running trivechain without having to do any manual configuration.
...
Configuration file:  `/etc/dashcore/trivechain.conf`  
...
dashcore user and group.  Access to trivechain-cli and other trivechain rpc clients
can then be controlled by group membership.

3b) Mac OS X

Binary:              `/usr/local/bin/dashd`  
Configuration file:  `~/Library/Application Support/DashCore/trivechain.conf`  
...
Rename dashd.openrc to trivechain and drop it in /etc/init.d.  Double
check ownership and permissions and make it executable.  Test it with
`/etc/init.d/trivechain start` and configure it to run on startup with
`rc-update add dashd`

4c) Upstart (for Debian/Ubuntu based distributions)

Drop dashd.conf in /etc/init.  Test by running `service trivechain start`
...
Copy dashd.init to /etc/init.d/dashd. Test by running `service trivechain start`.

Using this script, you can adjust the path and flags to the trivechain program by
...
This Launch Agent will cause trivechain to start whenever the user logs in.

NOTE: This approach is intended for those wanting to run trivechain as the current user.
