...
# trivechain_hash
RUN git clone https://github.com/trivechainpay/trivechain_hash
RUN cd trivechain_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} trivechain
RUN useradd -u ${USER_ID} -g trivechain -s /bin/bash -m -d /trivechain trivechain
...
RUN mkdir /trivechain-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /trivechain-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /trivechain-src
