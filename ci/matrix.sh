...
export BUILDER_IMAGE_NAME="trivechain-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/trivechain"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_TRIVECHAIN_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
