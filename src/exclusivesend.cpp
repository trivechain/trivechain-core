...
bool CDarksendQueue::Relay(CConnman& connman)
{
    ...
    BOOST_FOREACH(CNode* pnode, vNodesCopy)
        if(pnode->nVersion >= MIN_EXCLUSIVESEND_PEER_PROTO_VERSION)
...
void CExclusiveSendBase::SetNull()
{
    // Both sides
...
std::string CExclusiveSend::GetMessageByID(PoolMessage nMessageID)
{
    switch (nMessageID) {
        ...
        case ERR_RECENT:                return _("Last ExclusiveSend was too recent.");
...
void CExclusiveSend::SyncTransaction(const CTransaction& tx, const CBlock* pblock)
{
    ...
    if(pblock) {
        ...
        if(mi == mapBlockIndex.end() || !mi->second) {
            // shouldn't happen
            LogPrint("exclusivesend", "CExclusiveSendClient::SyncTransaction -- Failed to find block %s\n", blockHash.ToString());
            return;
        }
        pblockindex = mi->second;
    }
    mapDSTX[txHash].SetConfirmedHeight(pblockindex ? pblockindex->nHeight : -1);
    LogPrint("exclusivesend", "CExclusiveSendClient::SyncTransaction -- txid=%s\n", txHash.ToString());
}
