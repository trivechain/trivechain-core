...
uint256 CExclusiveSendQueue::GetSignatureHash() const
{
...
bool CExclusiveSendQueue::CheckSignature(const CBLSPublicKey& blsPubKey) const
{
    uint256 hash = GetSignatureHash();
...
bool CExclusiveSendQueue::Relay(CConnman& connman)
{
    connman.ForEachNode([&connman, this](CNode* pnode) {
        ...
        if (pnode->nVersion >= MIN_EXCLUSIVESEND_PEER_PROTO_VERSION && pnode->fSendDSQueue)
            connman.PushMessage(pnode, msgMaker.Make(NetMsgType::DSQUEUE, (*this)));
    });
    return true;
}

uint256 CExclusiveSendBroadcastTx::GetSignatureHash() const
{
    return SerializeHash(*this);
}
...
bool CExclusiveSendBroadcastTx::CheckSignature(const CBLSPublicKey& blsPubKey) const
{
...
void CExclusiveSendBaseSession::SetNull()
{
    // Both sides
    LOCK(cs_exclusivesend);
...
void CExclusiveSendBaseManager::SetNull()
{
    LOCK(cs_vecqueue);
    vecExclusiveSendQueue.clear();
}

void CExclusiveSendBaseManager::CheckQueue()
{
    ...
    while (it != vecExclusiveSendQueue.end()) {
        if ((*it).IsExpired()) {
            LogPrint("exclusivesend", "CExclusiveSendBaseManager::%s -- Removing expired queue (%s)\n", __func__, (*it).ToString());
...
bool CExclusiveSendBaseManager::GetQueueItemAndTry(CExclusiveSendQueue& dsqRet)
{
    TRY_LOCK(cs_vecqueue, lockDS);
    if (!lockDS) return false; // it's ok to fail here, we run this quite frequently

    for (auto& dsq : vecExclusiveSendQueue) {
...
std::string CExclusiveSendBaseSession::GetStateString() const
{
    switch (nState) {
...
bool CExclusiveSend::IsCollateralValid(const CTransaction& txCollateral)
{
    ...
    for (const auto& txin : txCollateral.vin) {
        ...
        if (mempoolTx != nullptr) {
            if (mempool.isSpent(txin.prevout) || !llmq::quorumDirectSendManager->IsLocked(txin.prevout.hash)) {
                LogPrint("exclusivesend", "CExclusiveSend::IsCollateralValid -- spent or non-locked mempool input! txin=%s\n", txin.ToString());
                return false;
            }
    ...
    if (nValueIn - nValueOut < GetCollateralAmount()) {
        LogPrint("exclusivesend", "CExclusiveSend::IsCollateralValid -- did not include enough fees in transaction: fees: %d, txCollateral=%s", nValueOut - nValueIn, txCollateral.ToString());
        return false;
    }

    LogPrint("exclusivesend", "CExclusiveSend::IsCollateralValid -- %s", txCollateral.ToString());

    {
        LOCK(cs_main);
        CValidationState validationState;
        if (!AcceptToMemoryPool(mempool, validationState, MakeTransactionRef(txCollateral), false, NULL, false, maxTxFee, true)) {
            LogPrint("exclusivesend", "CExclusiveSend::IsCollateralValid -- didn't pass AcceptToMemoryPool()\n");
            return false;
        }
    }
...
*/
std::string CExclusiveSend::GetDenominationsToString(int nDenom)
{
    std::string strDenom = "";
...
*/
int CExclusiveSend::GetDenominations(const std::vector<CTxOut>& vecTxOut, bool fSingleRandomDenom)
{
...
int CExclusiveSend::GetDenominationsByAmounts(const std::vector<CAmount>& vecAmount)
{
...
std::string CExclusiveSend::GetMessageByID(PoolMessage nMessageID)
{
    ...
    case ERR_RECENT:
        return _("Last ExclusiveSend was too recent.");
    case ERR_SESSION:
...
void CExclusiveSend::SyncTransaction(const CTransaction& tx, const CBlockIndex* pindex, int posInBlock)
{
    ...
    LogPrint("exclusivesend", "CExclusiveSend::SyncTransaction -- txid=%s\n", txHash.ToString());
}
