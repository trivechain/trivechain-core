...
#ifdef ENABLE_WALLET
UniValue exclusivesend(const JSONRPCRequest& request)
{
    ...
    if (request.params[0].get_str() == "start") {
        ...
        exclusiveSendClient.fEnableExclusiveSend = true;
        bool result = exclusiveSendClient.DoAutomaticDenominating(*g_connman);
        return "Mixing " + (result ? "started successfully" : ("start failed: " + exclusiveSendClient.GetStatuses() + ", will retry"));
    }
    ...
    return "Unknown command, please see \"help exclusivesend\"";
}
#endif // ENABLE_WALLET
...
UniValue getpoolinfo(const JSONRPCRequest& request)
{
...
#ifdef ENABLE_WALLET
    CExclusiveSendBaseManager* pexclusiveSendBaseManager = fMasternodeMode ? (CExclusiveSendBaseManager*)&exclusiveSendServer : (CExclusiveSendBaseManager*)&exclusiveSendClient;

    UniValue obj(UniValue::VOBJ);
    // TODO:
    // obj.push_back(Pair("state",             pexclusiveSendBase->GetStateString()));
    obj.push_back(Pair("queue",             pexclusiveSendBaseManager->GetQueueSize()));
    // obj.push_back(Pair("entries",           pexclusiveSendBase->GetEntriesCount()));
    obj.push_back(Pair("status",            exclusiveSendClient.GetStatuses()));

    std::vector<CDeterministicMNCPtr> vecDmns;
    if (exclusiveSendClient.GetMixingMasternodesInfo(vecDmns)) {
    ...
    if (pwallet) {
        ...
        obj.push_back(Pair("warnings",      pwallet->nKeysLeftSinceAutoBackup < EXCLUSIVESEND_KEYS_THRESHOLD_WARNING
                                                ? "WARNING: keypool is almost depleted!" : ""));
    }
#else // ENABLE_WALLET
    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("state",             exclusiveSendServer.GetStateString()));
    obj.push_back(Pair("queue",             exclusiveSendServer.GetQueueSize()));
    obj.push_back(Pair("entries",           exclusiveSendServer.GetEntriesCount()));
#endif // ENABLE_WALLET
...
void masternode_list_help()
{
    throw std::runtime_error(
            ...
            "  owneraddress   - Print the masternode owner Trivechain address\n"
            "  payee          - Print the masternode payout Trivechain address (can be additionally filtered,\n"
            "                   partial match)\n"
            "  pubKeyOperator - Print the masternode operator public key\n"
            "  status         - Print masternode status: ENABLED / POSE_BANNED\n"
            "                   (can be additionally filtered, partial match)\n"
            "  votingaddress  - Print the masternode voting Trivechain address\n"
        );
}
...
void masternode_count_help()
{
    throw std::runtime_error(
            ...
            "  ps            - number of ExclusiveSend compatible masternodes"
...
UniValue masternode_outputs(const JSONRPCRequest& request)
{
    ...
    pwallet->AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_10000);
