...
std::string EntryDescriptionString()
{
    return "    \"size\" : n,                 (numeric) transaction size in bytes\n"
           ...
           "       ... ],\n"
           "    \"directsend\" : true|false, (boolean) True if this transaction was sent as an DirectSend one\n"
           "    \"instantlock\" : true|false  (boolean) True if this transaction was locked via DirectSend\n";
}
...
void entryToJSON(UniValue &info, const CTxMemPoolEntry &e)
{
    ...
    info.push_back(Pair("directsend", directsend.HasTxLockRequest(tx.GetHash())));
    info.push_back(Pair("instantlock", directsend.IsLockedDirectSendTransaction(tx.GetHash()) || llmq::quorumDirectSendManager->IsLocked(tx.GetHash())));
}
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of trivechain addresses\n"
...
UniValue mempoolInfoToJSON()
{
    ...
    ret.push_back(Pair("directsendlocks", (int64_t)llmq::quorumDirectSendManager->GetDirectSendLockCount()));
...
UniValue getmempoolinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"directsendlocks\": xxxxx,   (numeric) Number of unconfirmed direct send locks\n"
            "}\n"
            "\nExamples:\n"
