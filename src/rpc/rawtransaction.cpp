...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry)
{
    ...
    bool fLocked = directsend.IsLockedDirectSendTransaction(txid);
    bool fLLMQLocked = llmq::quorumDirectSendManager->IsLocked(txid);
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the trivechain address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwG\"   (string) Trivechain address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue sendrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            "sendrawtransaction \"hexstring\" ( allowhighfees directsend bypasslimits)\n"
            "\nSubmits raw transaction (serialized, hex-encoded) to local node and network.\n"
            "\nAlso see createrawtransaction and signrawtransaction calls.\n"
            "\nArguments:\n"
            "1. \"hexstring\"    (string, required) The hex string of the raw transaction)\n"
            "2. allowhighfees  (boolean, optional, default=false) Allow high fees\n"
            "3. directsend    (boolean, optional, default=false) Use DirectSend to send this transaction\n"
    ...
    bool fDirectSend = false;
    if (request.params.size() > 2)
        fDirectSend = request.params[2].get_bool();
    ...
    if (!fHaveMempool && !fHaveChain) {
        ...
        if (fDirectSend && !directsend.ProcessTxLockRequest(*tx, *g_connman)) {
            throw JSONRPCError(RPC_TRANSACTION_ERROR, "Not a valid DirectSend transaction, see debug.log for more info");
        }
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "rawtransactions",    "sendrawtransaction",     &sendrawtransaction,     false, {"hexstring","allowhighfees","directsend","bypasslimits"} },
