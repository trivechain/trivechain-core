...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        // DirectSend Fee
        if (coinControl->fUseDirectSend) nPayFee = std::max(nPayFee, CTxLockRequest(txDummy).GetMinFee());
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            int nRounds = pwalletMain->GetOutpointExclusiveSendRounds(outpoint);

            if (nRounds >= 0 || fDebug) itemOutput->setText(COLUMN_EXCLUSIVESEND_ROUNDS, strPad(QString::number(nRounds), 11, " "));
            else itemOutput->setText(COLUMN_EXCLUSIVESEND_ROUNDS, strPad(QString(tr("n/a")), 11, " "));
