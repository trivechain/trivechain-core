...
QString TransactionDesc::FormatTxStatus(const CWalletTx& wtx)
{
    ...
    else
    {
        ...
        if (wtx.IsLockedByLLMQDirectSend()) {
            strTxStatus += " (" + tr("verified via LLMQ based DirectSend") + ")";
            return strTxStatus;
        }

        if(!directsend.HasTxLockRequest(wtx.GetHash())) return strTxStatus; // regular tx

        int nSignatures = directsend.GetTransactionLockSignatures(wtx.GetHash());
        int nSignaturesMax = CTxLockRequest(wtx).GetMaxSignatures();
        // DirectSend
        strTxStatus += " (";
        if(directsend.IsLockedDirectSendTransaction(wtx.GetHash())) {
            strTxStatus += tr("verified via DirectSend");
        } else if(!directsend.IsTxLockCandidateTimedOut(wtx.GetHash())) {
            strTxStatus += tr("DirectSend verification in progress - %1 of %2 signatures").arg(nSignatures).arg(nSignaturesMax);
        } else {
            strTxStatus += tr("DirectSend verification failed");
        }
        strTxStatus += ")";
...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    // Message from normal trivechain:URI (trivechain:XyZ...?message=example)
