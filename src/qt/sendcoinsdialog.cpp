    ...
    platformStyle(_platformStyle)
{
    ...
    if(fLiteMode) {
        ...
        ui->checkUseExclusiveSend->setVisible(false);
        ui->checkUseDirectSend->setVisible(false);
        CoinControlDialog::coinControl->fUseExclusiveSend = false;
        CoinControlDialog::coinControl->fUseDirectSend = false;
    }
    else{
        ui->checkUseExclusiveSend->setChecked(fUseExclusiveSend);
        ui->checkUseDirectSend->setChecked(fUseDirectSend);
        CoinControlDialog::coinControl->fUseExclusiveSend = fUseExclusiveSend;
        CoinControlDialog::coinControl->fUseDirectSend = fUseDirectSend;
    }

    connect(ui->checkUseExclusiveSend, SIGNAL(stateChanged ( int )), this, SLOT(updateDisplayUnit()));
    connect(ui->checkUseDirectSend, SIGNAL(stateChanged ( int )), this, SLOT(updateDirectSend()));
    ...
    if(ui->checkUseExclusiveSend->isChecked()) {
        ...
        QString strNearestAmount(
            BitcoinUnits::formatWithUnit(
                model->getOptionsModel()->getDisplayUnit(), CExclusiveSend::GetSmallestDenomination()));
        strFee = QString(tr(
            "(exclusivesend requires this amount to be rounded up to the nearest %1)."
        ).arg(strNearestAmount));
    } else {
        strFunds = tr("using") + " <b>" + tr("any available funds (not anonymous)") + "</b>";
    }

    if(model->IsOldDirectSendEnabled() && ui->checkUseDirectSend->isChecked()) {
        strFunds += " ";
        strFunds += tr("and DirectSend");
    }

    for (SendCoinsRecipient& rcp : recipients) {
        rcp.inputType = ui->checkUseExclusiveSend->isChecked() ? ONLY_DENOMINATED : ALL_COINS;
        rcp.fUseDirectSend = model->IsOldDirectSendEnabled() && ui->checkUseDirectSend->isChecked();
    }
...
void SendCoinsDialog::updateDisplayUnit()
{
    ...
    CoinControlDialog::coinControl->fUseExclusiveSend = ui->checkUseExclusiveSend->isChecked();
    coinControlUpdateLabels();
...
void SendCoinsDialog::updateDirectSend()
{
    QSettings settings;
    settings.setValue("bUseInstantX", ui->checkUseDirectSend->isChecked());
    CoinControlDialog::coinControl->fUseDirectSend = model->IsOldDirectSendEnabled() && ui->checkUseDirectSend->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::coinControlUpdateLabels()
{
    ...
    ui->checkUseExclusiveSend->setChecked(CoinControlDialog::coinControl->fUseExclusiveSend);
