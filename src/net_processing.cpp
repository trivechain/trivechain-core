...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    */
    case MSG_TXLOCK_REQUEST:
        return directsend.AlreadyHave(inv.hash);
    ...
    case MSG_DSTX: {
        return static_cast<bool>(CExclusiveSend::GetDSTX(inv.hash));
    }
    ...
    case MSG_ISLOCK:
        return llmq::quorumDirectSendManager->AlreadyHave(inv);
    }
    ...
    const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
    {
        ...
        while (it != pfrom->vRecvGetData.end() && it->IsKnownType()) {
            ...
            if (!push && inv.type == MSG_TXLOCK_REQUEST) {
                ...
                if(directsend.GetTxLockRequest(inv.hash, txLockRequest)) {
            ...
            if (!push && inv.type == MSG_TXLOCK_VOTE) {
                CTxLockVote vote;
                if(directsend.GetTxLockVote(inv.hash, vote)) {
            ...
            if (!push && (inv.type == MSG_ISLOCK)) {
                llmq::CDirectSendLock o;
                if (llmq::quorumDirectSendManager->GetDirectSendLockByHash(inv.hash, o)) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (nTime > TIME_PEER_PROTO_CHECK && nVersion < TIME_MIN_PEER_PROTO_VERSION) {
            // disconnect from peers older than this proto version
            LogPrintf("peer=%d using obsolete version %i; disconnecting\n", pfrom->id, nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                               strprintf("Version must be %d or greater after timestamp %d", TIME_MIN_PEER_PROTO_VERSION, TIME_PEER_PROTO_CHECK)));
            pfrom->fDisconnect = true;
            return false;
        }
    ...
    if (strCommand == NetMsgType::VERACK)
    {
        ...
        if (pfrom->nVersion >= SENDDSQUEUE_PROTO_VERSION) {
            // Tell our peer that he should send us ExclusiveSend queue messages
            connman.PushMessage(pfrom, msgMaker.Make(NetMsgType::SENDDSQUEUE, true));
        } else {
            // older nodes do not support SENDDSQUEUE and expect us to always send ExclusiveSend queue messages
        ...
        if (pfrom->nVersion >= LLMQS_PROTO_VERSION) {
            ...
            // e.g. DirectSend locks or ChainLocks. SPV nodes should not send this message
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX || strCommand == NetMsgType::TXLOCKREQUEST)
    {
        ...
        if(strCommand == NetMsgType::TX) {
            ...
            fCanAutoLock = llmq::IsOldDirectSendEnabled() && CDirectSend::CanAutoLock() && txLockRequest.IsSimple();
        } else if(strCommand == NetMsgType::TXLOCKREQUEST) {
            vRecv >> txLockRequest;
            ptx = txLockRequest.tx;
            nInvType = MSG_TXLOCK_REQUEST;
            if (llmq::IsNewDirectSendEnabled()) {
        ...
        if (nInvType == MSG_TXLOCK_REQUEST || fCanAutoLock) {
            if(!directsend.ProcessTxLockRequest(txLockRequest, connman)) {
                LogPrint("directsend", "TXLOCKREQUEST -- failed %s\n", txLockRequest.GetHash().ToString());
            ...
            if(CExclusiveSend::GetDSTX(hashTx)) {
                LogPrint("exclusivesend", "DSTX -- Already have %s, skipping...\n", hashTx.ToString());
                return true; // not an error
            }

            auto dmn = deterministicMNManager->GetListAtChainTip().GetMNByCollateral(dstx.masternodeOutpoint);
            if(!dmn) {
                LogPrint("exclusivesend", "DSTX -- Can't find masternode %s to verify %s\n", dstx.masternodeOutpoint.ToStringShort(), hashTx.ToString());
                return false;
            }

            if (!mmetaman.GetMetaInfo(dmn->proTxHash)->IsValidForMixingTxes()) {
                LogPrint("exclusivesend", "DSTX -- Masternode %s is sending too many transactions %s\n", dstx.masternodeOutpoint.ToStringShort(), hashTx.ToString());
                return true;
            ...
            if (!dstx.CheckSignature(dmn->pdmnState->pubKeyOperator.Get())) {
                LogPrint("exclusivesend", "DSTX -- CheckSignature() failed for %s\n", hashTx.ToString());
                return false;
            }
        ...
        else if (fMissingInputs)
        {
            ...
            if (nInvType == MSG_TXLOCK_REQUEST && !AlreadyHave(inv)) {
                ...
                directsend.RejectLockRequest(txLockRequest);
    ...
    else if (strCommand == NetMsgType::FILTERCLEAR)
    {
        ...
        if (found)
        {
            ...
            llmq::quorumDirectSendManager->ProcessMessage(pfrom, strCommand, vRecv, connman);
        }
        else
        {
