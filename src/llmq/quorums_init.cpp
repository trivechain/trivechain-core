...
void InitLLMQSystem(CEvoDB& evoDb, CScheduler* scheduler, bool unitTests, bool fWipe)
{
    ...
    quorumDirectSendManager = new CDirectSendManager(*llmqDb);
}

void DestroyLLMQSystem()
{
    delete quorumDirectSendManager;
    quorumDirectSendManager = nullptr;
...
void StartLLMQSystem()
{
    ...
    if (quorumDirectSendManager) {
        quorumDirectSendManager->Start();
    }
}
...
void InterruptLLMQSystem()
{
    ...
    if (quorumDirectSendManager) {
        quorumDirectSendManager->InterruptWorkerThread();
    }
}
