...
void CChainLocksHandler::TrySignChainTip()
{
    ...
    if (IsNewDirectSendEnabled() && sporkManager.IsSporkActive(SPORK_3_DIRECTSEND_BLOCK_FILTERING)) {
        ...
        while (pindexWalk) {
            ...
            for (auto& txid : *txids) {
                ...
                if (txAge < WAIT_FOR_ISLOCK_TIMEOUT && !quorumDirectSendManager->IsLocked(txid)) {
