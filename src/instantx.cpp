...
std::atomic<bool> CDirectSend::isAutoLockBip9Active{false};
const double CDirectSend::AUTO_IX_MEMPOOL_THRESHOLD = 0.1;

CDirectSend directsend;
const std::string CDirectSend::SERIALIZATION_VERSION_STRING = "CDirectSend-Version-1";

// Transaction Locks
//
// step 1) Some node announces intention to lock transaction inputs via "txlockrequest" message (ix)
// step 2) Top nDirectSendSigsTotal masternodes per each spent outpoint push "txlockvote" message (txlvote)
// step 3) Once there are nDirectSendSigsRequired valid "txlockvote" messages (txlvote) per each spent outpoint
...
void CDirectSend::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    if (fLiteMode) return; // disable all Dash specific functionality
    if (!llmq::IsOldDirectSendEnabled()) return;
    ...
    if (strCommand == NetMsgType::TXLOCKVOTE) { // DirectSend Transaction Lock Consensus Votes
        if(pfrom->nVersion < MIN_DIRECTSEND_PROTO_VERSION) {
            LogPrint("directsend", "TXLOCKVOTE -- peer=%d using obsolete version %i\n", pfrom->id, pfrom->nVersion);
            connman.PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                               strprintf("Version must be %d or greater", MIN_DIRECTSEND_PROTO_VERSION)));
            return;
        }
...
#endif
    LOCK2(mempool.cs, cs_directsend);
            ...
            for (const auto& hash : it->second) {
                if (hash != txLockRequest.GetHash()) {
                    ...
                    if (itPrevious != mapTxLockCandidates.end() && itPrevious->second.txLockRequest) {
                        ...
                        GetMainSignals().NotifyDirectSendDoubleSpendAttempt(txCurrent, txPrevious);
                    }
                }
            }
        }
...
void CDirectSend::Vote(const uint256& txHash, CConnman& connman)
{
...
void CDirectSend::Vote(CTxLockCandidate& txLockCandidate, CConnman& connman)
{
    ...
    for (auto& outpointLockPair : txLockCandidate.mapOutPointLocks) {
        ...
        int nLockInputHeight = nPrevoutHeight + Params().GetConsensus().nDirectSendConfirmationsRequired - 2;
        ...
        int nSignaturesTotal = Params().GetConsensus().nDirectSendSigsTotal;
...
bool CDirectSend::ProcessNewTxLockVote(CNode* pfrom, const CTxLockVote& vote, CConnman& connman)
{
    ...
    if (!vote.IsValid(pfrom, connman)) {
        ...
        LogPrint("directsend", "CDirectSend::%s -- Vote is invalid, txid=%s\n", __func__, txHash.ToString());
        return false;
    }
    ...
    if (it == mapTxLockCandidates.end() || !it->second.txLockRequest) {
        ...
        LogPrint("directsend", "CDirectSend::%s -- Orphan vote: txid=%s  masternode=%s %s\n",
        ...
        if (itMnOV == mapMasternodeOrphanVotes.end()) {
            ...
            if (itMnOV->second > GetTime() && itMnOV->second > GetAverageMasternodeOrphanVoteTime()) {
                LogPrint("directsend", "CDirectSend::%s -- masternode is spamming orphan Transaction Lock Votes: txid=%s  masternode=%s\n",
    ...
    if (txLockCandidate.IsTimedOut()) {
        LogPrint("directsend", "CDirectSend::%s -- too late, Transaction Lock timed out, txid=%s\n", __func__, txHash.ToString());
        return false;
    }

    LogPrint("directsend", "CDirectSend::%s -- Transaction Lock Vote, txid=%s\n", __func__, txHash.ToString());
    ...
    LogPrint("directsend", "CDirectSend::%s -- Transaction Lock signatures count: %d/%d, vote hash=%s\n", __func__,
...
bool CDirectSend::ProcessOrphanTxLockVote(const CTxLockVote& vote)
{
...
void CDirectSend::UpdateVotedOutpoints(const CTxLockVote& vote, CTxLockCandidate& txLockCandidate)
{
    AssertLockHeld(cs_directsend);
        ...
        for (const auto& hash : it1->second) {
            if (hash != txHash) {
                ...
                if (it2 !=mapTxLockCandidates.end() && it2->second.HasMasternodeVoted(vote.GetOutpoint(), vote.GetMasternodeOutpoint())) {
                    ...
                    LogPrintf("CDirectSend::%s -- masternode sent conflicting votes! %s\n", __func__, vote.GetMasternodeOutpoint().ToStringShort());
...
void CDirectSend::ProcessOrphanTxLockVotes()
{
    AssertLockHeld(cs_main);
    AssertLockHeld(cs_directsend);
...
{
    if (!fEnableDirectSend || GetfLargeWorkForkFound() || GetfLargeWorkInvalidChainFound() ||
...
int CDirectSend::GetTransactionLockSignatures(const uint256& txHash)
{
    ...
    if (!llmq::IsOldDirectSendEnabled()) return -3;
...
void CDirectSend::SyncTransaction(const CTransaction& tx, const CBlockIndex *pindex, int posInBlock)
{
...
bool CDirectSend::CanAutoLock()
{
    if (!isAutoLockBip9Active || !llmq::IsOldDirectSendEnabled()) {
        return false;
    }
    if (!sporkManager.IsSporkActive(SPORK_16_DIRECTSEND_AUTOLOCKS)) {
        return false;
    }
...
bool CTxLockRequest::IsValid() const
{
    ...
    for (const auto& txin : tx->vin) {
        ...
        int nConfirmationsRequired = nDirectSendConfirmationsRequired - 1;
...
{
    if (!fForceMinFee && CDirectSend::CanAutoLock() && IsSimple()) {
        return CAmount();
    }
...
{
    return tx->vin.size() * Params().GetConsensus().nDirectSendSigsTotal;
}
...
bool CTxLockVote::IsValid(CNode* pnode, CConnman& connman) const
{
    ...
    if (!masternodeProTxHash.IsNull()) {
        ...
        if (!dmn || dmn->collateralOutpoint != outpointMasternode) {
            LogPrint("directsend", "CTxLockVote::IsValid -- invalid masternodeProTxHash %s\n", masternodeProTxHash.ToString());
            return false;
        }
    } else {
        LogPrint("directsend", "CTxLockVote::IsValid -- missing masternodeProTxHash\n");
        return false;
    }
    ...
    int nLockInputHeight = coin.nHeight + Params().GetConsensus().nDirectSendConfirmationsRequired - 2;
    ...
    if (!quorumModifierHash.IsNull()) {
        if (quorumModifierHash != expectedQuorumModifierHash) {
            LogPrint("directsend", "CTxLockVote::IsValid -- invalid quorumModifierHash %s, expected %s\n", quorumModifierHash.ToString(), expectedQuorumModifierHash.ToString());
            return false;
        }
    } else {
        LogPrint("directsend", "CTxLockVote::IsValid -- missing quorumModifierHash\n");
        return false;
    }
...
void CTxLockVote::Relay(CConnman& connman) const
{
    ...
    if (directsend.GetTxLockRequest(txHash, request) && request) {
...
{
    return !fAttacked && CountVotes() >= Params().GetConsensus().nDirectSendSigsRequired;
}
