...
inline int set_error(trivechainconsensus_error* ret, trivechainconsensus_error serror)
{
    if (ret)
        *ret = serror;
    return 0;
                                    ...
                                    unsigned int nIn, unsigned int flags, trivechainconsensus_error* err)
{
    if (!verify_flags(flags)) {
        return trivechainconsensus_ERR_INVALID_FLAGS;
    }
    try {
        ...
        if (GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) != txToLen)
         ...
         set_error(err, trivechainconsensus_ERR_OK);

        return VerifyScript(tx.vin[nIn].scriptSig, CScript(scriptPubKey, scriptPubKey + scriptPubKeyLen), flags, TransactionSignatureChecker(&tx, nIn), NULL);
    } catch (const std::exception&) {
        return set_error(err, trivechainconsensus_ERR_TX_DESERIALIZE); // Error deserializing
    }
}

unsigned int trivechainconsensus_version()
{
