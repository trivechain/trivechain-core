...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    // should be at least not earlier than block when 1000 TRVC tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 TRVC tx -> 1 confirmation
