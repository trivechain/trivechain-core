...
void PrepareShutdown()
{
    ...
    RenameThread("trivecoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "trivecoin (or specifically: gobject, directsend, keepass, masternode, mnpayments, mnsync, exclusivesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all TriveCoin specific functionality (Masternodes, ExclusiveSend, DirectSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-directsenddepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), DEFAULT_DIRECTSEND_DEPTH));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The TriveCoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("trivecoin-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that TriveCoin is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if(!GetBoolArg("-enabledirectsend", fEnableDirectSend)){
        if (SoftSetArg("-directsenddepth", 0))
    ...
    if (nLiqProvTmp > 0) {
        ...
        mapArgs["-exclusivesendrounds"] = "99999";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -exclusivesendrounds=99999\n", __func__, nLiqProvTmp);
        mapArgs["-exclusivesendamount"] = "999999";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -exclusivesendamount=999999\n", __func__, nLiqProvTmp);
        mapArgs["-exclusivesendmultisession"] = "0";
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("TriveCoin Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize TriveCoin.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. TriveCoin is shutting down."));
...
#endif
    // Make sure only a single TriveCoin process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. TriveCoin is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. TriveCoin is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of TriveCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart TriveCoin to complete") << "\n";
    ...
    // ********************************************************* Step 11c: update block tip in TriveCoin modules
    ...
    // ********************************************************* Step 11d: start trivecoin-ps-<smth> threads
