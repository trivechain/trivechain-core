...
void PrepareShutdown()
{
    ...
    if (!fLiteMode && !fRPCInWarmup) {
        // Stop ExclusiveSend, release keys
    ...
    if (!fLiteMode && !fRPCInWarmup) {
        ...
        if(fEnableDirectSend)
        {
            CFlatDB<CDirectSend> flatdb5("directsend.dat", "magicDirectSendCache");
            flatdb5.Dump(directsend);
        }
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via DirectSend) in <address>"));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashdirectsenddoublespend=<address>", _("Enable publish transaction hashes of attempted DirectSend double spend in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via DirectSend) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawdirectsenddoublespend=<address>", _("Enable publish raw transactions of attempted DirectSend double spend in <address>"));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode", strprintf(_("Disable all Trivechain specific functionality (Masternodes, ExclusiveSend, DirectSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-enableexclusivesend", strprintf(_("Enable use of automated ExclusiveSend for funds stored in this wallet (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-enabledirectsend", strprintf(_("Enable DirectSend, show confirmations for locked transactions (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-directsendnotify=<cmd>", _("Execute command when a wallet DirectSend transaction is successfully locked (%s in cmd is replaced by TxID)"));
...
void InitParameterInteraction()
{
    ...
    if (nLiqProvTmp > 0) {
        ForceSetArg("-enableexclusivesend", "1");
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -enableexclusivesend=1\n", __func__, nLiqProvTmp);
        ForceSetArg("-exclusivesendsessions", itostr(MIN_EXCLUSIVESEND_SESSIONS));
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -exclusivesendsessions=%d\n", __func__, nLiqProvTmp, itostr(std::numeric_limits<int>::max()));
        ForceSetArg("-exclusivesendrounds", itostr(std::numeric_limits<int>::max()));
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -exclusivesendrounds=%d\n", __func__, nLiqProvTmp, itostr(std::numeric_limits<int>::max()));
        ForceSetArg("-exclusivesendamount", itostr(MAX_EXCLUSIVESEND_AMOUNT));
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -exclusivesendamount=%d\n", __func__, nLiqProvTmp, MAX_EXCLUSIVESEND_AMOUNT);
        ForceSetArg("-exclusivesenddenoms", itostr(MAX_EXCLUSIVESEND_DENOMS));
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -exclusivesenddenoms=%d\n", __func__, nLiqProvTmp, itostr(MAX_EXCLUSIVESEND_DENOMS));
        ForceSetArg("-exclusivesendmultisession", "0");
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -exclusivesendmultisession=0\n", __func__, nLiqProvTmp);
    }
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    // lite mode disables all Trivechain-specific functionality
    fLiteMode = GetBoolArg("-litemode", false);
    LogPrintf("fLiteMode %d\n", fLiteMode);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all Trivechain-specific functionality is disabled."));
    }
    ...
    // ********************************************************* Step 10b: setup ExclusiveSend

#ifdef ENABLE_WALLET
    exclusiveSendClient.nLiquidityProvider = std::min(std::max((int)GetArg("-liquidityprovider", DEFAULT_EXCLUSIVESEND_LIQUIDITY), MIN_EXCLUSIVESEND_LIQUIDITY), MAX_EXCLUSIVESEND_LIQUIDITY);
    int nMaxRounds = MAX_EXCLUSIVESEND_ROUNDS;
    ...
    exclusiveSendClient.nExclusiveSendSessions = std::min(std::max((int)GetArg("-exclusivesendsessions", DEFAULT_EXCLUSIVESEND_SESSIONS), MIN_EXCLUSIVESEND_SESSIONS), MAX_EXCLUSIVESEND_SESSIONS);
    exclusiveSendClient.nExclusiveSendRounds = std::min(std::max((int)GetArg("-exclusivesendrounds", DEFAULT_EXCLUSIVESEND_ROUNDS), MIN_EXCLUSIVESEND_ROUNDS), nMaxRounds);
    exclusiveSendClient.nExclusiveSendAmount = std::min(std::max((int)GetArg("-exclusivesendamount", DEFAULT_EXCLUSIVESEND_AMOUNT), MIN_EXCLUSIVESEND_AMOUNT), MAX_EXCLUSIVESEND_AMOUNT);
    exclusiveSendClient.nExclusiveSendDenoms = std::min(std::max((int)GetArg("-exclusivesenddenoms", DEFAULT_EXCLUSIVESEND_DENOMS), MIN_EXCLUSIVESEND_DENOMS), MAX_EXCLUSIVESEND_DENOMS);

    LogPrintf("ExclusiveSend liquidityprovider: %d\n", exclusiveSendClient.nLiquidityProvider);
    ...
    LogPrintf("ExclusiveSend denoms: %d\n", exclusiveSendClient.nExclusiveSendDenoms);
#endif // ENABLE_WALLET

    CExclusiveSend::InitStandardDenominations();

    // ********************************************************* Step 10b: setup DirectSend
    ...
    if (!fIgnoreCacheFiles) {
        ...
        if(fEnableDirectSend)
        {
            strDBName = "directsend.dat";
            uiInterface.InitMessage(_("Loading DirectSend data cache..."));
            CFlatDB<CDirectSend> flatdb5(strDBName, "magicDirectSendCache");
            if(!flatdb5.Load(directsend)) {
                return InitError(_("Failed to load DirectSend data cache from") + "\n" + (pathDB / strDBName).string());
            }
        }
    }

    // ********************************************************* Step 10d: schedule Trivechain-specific tasks
    ...
    if (!fLiteMode) {
        ...
        scheduler.scheduleEvery(boost::bind(&CDirectSend::DoMaintenance, boost::ref(directsend)), 60 * 1000);

        if (fMasternodeMode)
            scheduler.scheduleEvery(boost::bind(&CExclusiveSendServer::DoMaintenance, boost::ref(exclusiveSendServer), boost::ref(*g_connman)), 1 * 1000);
#ifdef ENABLE_WALLET
        else
            scheduler.scheduleEvery(boost::bind(&CExclusiveSendClientManager::DoMaintenance, boost::ref(exclusiveSendClient), boost::ref(*g_connman)), 1 * 1000);
#endif // ENABLE_WALLET
    }
