...
extern CDirectSend directsend;

/*
    At 15 signatures, 1/2 of the masternode network can be owned by
    one party without compromising the security of DirectSend
...
static const int MIN_DIRECTSEND_PROTO_VERSION      = 70213;

/// For how long we are going to accept votes/locks
/// after we saw the first one for a specific transaction
static const int DIRECTSEND_LOCK_TIMEOUT_SECONDS   = 15;
/// For how long we are going to keep invalid votes and votes for failed lock attempts,
/// must be greater than DIRECTSEND_LOCK_TIMEOUT_SECONDS
static const int DIRECTSEND_FAILED_TIMEOUT_SECONDS = 60;

extern bool fEnableDirectSend;

/**
 * Manages DirectSend. Processes lock requests, candidates, and votes.
 */
class CDirectSend
{
public:
...
public:
    mutable CCriticalSection cs_directsend;
    ...
    bool IsLockedDirectSendTransaction(const uint256& txHash);
...
/**
 * An DirectSend transaction lock request.
 */
class CTxLockRequest
{
private:
...
/**
 * An DirectSend transaction lock vote. Sent by a masternode in response to a
...
/**
 * An DirectSend transaction lock candidate.
 */
class CTxLockCandidate
{
private:
