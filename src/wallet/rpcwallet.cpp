...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"trivechainaddress\"    (string) The new trivechain address\n"
            "\nExamples:\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"trivechainaddress\"   (string) The account trivechain address\n"
            "\nExamples:\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"trivechainaddress\"  (string, required) The trivechain address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"trivechainaddress\"  (string, required) The trivechain address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"trivechainaddress\"  (string) a trivechain address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
static void SendMoney(const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, bool fUseDirectSend=false, bool fUseExclusiveSend=false)
{
    ...
    if (!pwalletMain->CommitTransaction(wtxNew, reservekey, g_connman.get(), fUseDirectSend ? NetMsgType::TXLOCKREQUEST : NetMsgType::TX))
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"trivechainaddress\" (string, required) The trivechain address to send to.\n"
            ...
            "6. \"use_is\"      (bool, optional) Send this transaction as DirectSend (default: false)\n"
    ...
    if (params.size() > 5)
        fUseDirectSend = params[5].get_bool();
    if (params.size() > 6)
        fUseExclusiveSend = params[6].get_bool();

    EnsureWalletIsUnlocked();

    SendMoney(address.Get(), nAmount, fSubtractFeeFromAmount, wtx, fUseDirectSend, fUseExclusiveSend);

    return wtx.GetHash().GetHex();
}

UniValue directsendtoaddress(const UniValue& params, bool fHelp)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"trivechainaddress\",     (string) The trivechain address\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"trivechainaddress\"  (string, required) The trivechain address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            ...
            "\nReturns the total amount received by the given trivechainaddress in transactions with specified minimum number of confirmations.\n"
            "\nArguments:\n"
            "1. \"trivechainaddress\"  (string, required) The trivechain address for transactions.\n"
            "2. minconf        (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "3. addlockconf    (bool, optional, default=false) Whether to add " + std::to_string(nDirectSendDepth) + " confirmations to transactions locked via DirectSend.\n"
            "\nResult:\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            ...
            "2. \"totrivechainaddress\"  (string, required) The trivechain address to send funds to.\n"
            "3. amount           (numeric or string, required) The amount in " + CURRENCY_UNIT + " (transaction fee is added on top).\n"
            "4. minconf          (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            "5. addlockconf      (bool, optional, default=false) Whether to add " + std::to_string(nDirectSendDepth) + " confirmations to transactions locked via DirectSend.\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 8)
        throw runtime_error(
            ...
            "4. addlockconf             (bool, optional, default=false) Whether to add " + std::to_string(nDirectSendDepth) + " confirmations to transactions locked via DirectSend.\n"
            ...
            "      ,...\n"
            "    ]\n"
            "7. \"use_is\"                (bool, optional) Send this transaction as DirectSend (default: false)\n"
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Trivechain address: ")+name_);
    ...
    bool fUseExclusiveSend = false;
    if (params.size() > 6)
        fUseDirectSend = params[6].get_bool();
    if (params.size() > 7)
        fUseExclusiveSend = params[7].get_bool();

    bool fCreated = pwalletMain->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, nChangePosRet, strFailReason,
                                                   NULL, true, fUseExclusiveSend ? ONLY_DENOMINATED : ALL_COINS, fUseDirectSend);
    if (!fCreated)
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, strFailReason);
    if (!pwalletMain->CommitTransaction(wtx, keyChange, g_connman.get(), fUseDirectSend ? NetMsgType::TXLOCKREQUEST : NetMsgType::TX))
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "2. \"keysobject\"   (string, required) A json array of trivechain addresses or hex-encoded public keys\n"
            "     [\n"
            ...
            "\nResult:\n"
            "\"trivechainaddress\"  (string) A trivechain address associated with the keys.\n"
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "2. addlockconf      (bool, optional, default=false) Whether to add " + std::to_string(nDirectSendDepth) + " confirmations to transactions locked via DirectSend.\n"
            ...
            "                                                 including additional " + std::to_string(nDirectSendDepth) + " confirmations for transactions locked via DirectSend\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"trivechainaddress\",  (string) The trivechain address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"trivechainaddress\",  (string) The trivechain address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"trivechainaddress\",      (string) The trivechain address involved in the transaction\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            + HelpExampleCli("signmessage", "\"trivechainaddress\" \"test message\"") +
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Trivechain address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        entry.push_back(Pair("ps_rounds", pwalletMain->GetOutpointExclusiveSendRounds(COutPoint(out.tx->GetHash(), out.i))));
