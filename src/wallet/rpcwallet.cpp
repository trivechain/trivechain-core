...
void WalletTxToJSON(const CWalletTx& wtx, UniValue& entry)
{
    ...
    bool fLocked = directsend.IsLockedDirectSendTransaction(wtx.GetHash());
    bool fLLMQLocked = llmq::quorumDirectSendManager->IsLocked(wtx.GetHash());
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new Trivechain address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"    (string) The new trivechain address\n"
            "\nExamples:\n"
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current Trivechain address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account trivechain address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Trivechain address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The trivechain address to be associated with an account.\n"
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Trivechain address");
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The trivechain address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a trivechain address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
static void SendMoney(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, bool fUseDirectSend = false, bool fUseExclusiveSend = false)
{
    ...
    if (!pwallet->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet,
                                         strError, NULL, true, fUseExclusiveSend ? ONLY_DENOMINATED : ALL_COINS, fUseDirectSend)) {
    ...
    if (fUseDirectSend && llmq::IsOldDirectSendEnabled()) {
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The trivechain address to send to.\n"
            ...
            "                             The recipient will receive less amount of Trivechain than you enter in the amount field.\n"
            "6. \"use_is\"             (bool, optional, default=false) Send this transaction as DirectSend\n"
    ...
    bool fUseDirectSend = false;
    bool fUseExclusiveSend = false;
    if (request.params.size() > 5)
        fUseDirectSend = request.params[5].get_bool();
    if (request.params.size() > 6)
        fUseExclusiveSend = request.params[6].get_bool();

    EnsureWalletIsUnlocked(pwallet);

    SendMoney(pwallet, address.Get(), nAmount, fSubtractFeeFromAmount, wtx, fUseDirectSend, fUseExclusiveSend);

    return wtx.GetHash().GetHex();
}

UniValue directsendtoaddress(const JSONRPCRequest& request)
{
    CWallet* const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            "directsendtoaddress \"address\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("directsendtoaddress", "\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwG\" 0.1")
            + HelpExampleCli("directsendtoaddress", "\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwG\" 0.1 \"donation\" \"seans outpost\"")
            + HelpExampleCli("directsendtoaddress", "\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwG\" 0.1 \"\" \"\" true")
            + HelpExampleRpc("directsendtoaddress", "\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwG\", 0.1, \"donation\", \"seans outpost\"")
        );
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The trivechain address\n"
...
UniValue listaddressbalances(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"address\": amount,       (string) The trivechain address and the amount in " + CURRENCY_UNIT + "\n"
            "  ,...\n"
            "}\n"
            "\nExamples:\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The trivechain address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The trivechain address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "3. addlocked           (bool, optional, default=false) Whether to include transactions locked via DirectSend.\n"
            "\nResult:\n"
    ...
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        BOOST_FOREACH(const CTxOut& txout, wtx.tx->vout)
            if (txout.scriptPubKey == scriptPubKey)
                if ((wtx.GetDepthInMainChain() >= nMinDepth) || (fAddLocked && wtx.IsLockedByDirectSend()))
...
UniValue getbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "3. addlocked      (bool, optional, default=false) Whether to include the value of transactions locked via DirectSend in the wallet's balance.\n"
    ...
    CAmount nBalance = pwallet->GetAccountBalance(strAccount, nMinDepth, filter, fAddLocked);
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a trivechain address."
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The trivechain address to send funds to.\n"
            "3. amount              (numeric or string, required) The amount in " + CURRENCY_UNIT + " (transaction fee is added on top).\n"
            "4. minconf             (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            "5. addlocked         (bool, optional, default=false) Whether to include transactions locked via DirectSend.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount     (numeric or string) The trivechain address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            "3. minconf                 (numeric, optional, default=1) Only use the balance confirmed at least this many times.\n"
            "4. addlocked               (bool, optional, default=false) Whether to include transactions locked via DirectSend.\n"
            "5. \"comment\"               (string, optional) A comment\n"
            "6. subtractfeefromamount   (array, optional) A json array with addresses.\n"
            "                           The fee will be equally deducted from the amount of each selected address.\n"
            "                           Those recipients will receive less trivechains than you enter in their corresponding amount field.\n"
            "                           If no addresses are specified here, the sender pays the fee.\n"
            "    [\n"
            "      \"address\"          (string) Subtract fee from this address\n"
            "      ,...\n"
            "    ]\n"
            "7. \"use_is\"                (bool, optional, default=false) Send this transaction as DirectSend\n"
    ...
    BOOST_FOREACH(const std::string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Trivechain address: ")+name_);
    ...
    bool fUseExclusiveSend = false;
    if (request.params.size() > 6)
        fUseDirectSend = request.params[6].get_bool();
    if (request.params.size() > 7)
        fUseExclusiveSend = request.params[7].get_bool();

    bool fCreated = pwallet->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, nChangePosRet, strFailReason,
                                               NULL, true, fUseExclusiveSend ? ONLY_DENOMINATED : ALL_COINS, fUseDirectSend);
    if (!fCreated)
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a Trivechain address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keys\"         (string, required) A json array of trivechain addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) trivechain address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"address\"         (string) A trivechain address associated with the keys.\n"
...
UniValue ListReceived(CWallet * const pwallet, const UniValue& params, bool fByAccounts)
{
    ...
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        if ((nDepth < nMinDepth) && !(fAddLocked && wtx.IsLockedByDirectSend()))
            continue;
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "2. addlocked         (bool, optional, default=false) Whether to include transactions locked via DirectSend.\n"
            ...
            "                                                 configured for transactions locked via DirectSend\n"
...
void ListTransactions(CWallet * const pwallet, const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const COutputEntry& s, listSent)
        {
            ...
            entry.push_back(Pair("category", (it != wtx.mapValue.end() && it->second == "1") ? "exclusivesend" : "send"));
    ...
    // Received
    if (listReceived.size() > 0 && ((wtx.GetDepthInMainChain() >= nMinDepth) || wtx.IsLockedByDirectSend()))
    {
        BOOST_FOREACH(const COutputEntry& r, listReceived)
        {
            std::string account;
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The trivechain address of the transaction. Not present for \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The trivechain address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "      \"address\" : \"address\",          (string) The trivechain address involved in the transaction\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (pwallet->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3)) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending trivechains\n"
            "\nArguments:\n"
            ...
            "\nUnlock the wallet for 60 seconds but allow ExclusiveSend mixing only\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->IsCrypted() && (request.fHelp || request.params.size() != 1)) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending trivechain\n"
    ...
    StartShutdown();
    return "Wallet encrypted; Trivechain server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending trivechains.\n"
...
UniValue setexclusivesendrounds(const JSONRPCRequest& request)
{
    CWallet* const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "setexclusivesendrounds rounds\n"
            "\nSet the number of rounds for ExclusiveSend mixing.\n"
            "\nArguments:\n"
            "1. rounds         (numeric, required) The default number of rounds is " + std::to_string(DEFAULT_EXCLUSIVESEND_ROUNDS) + 
            " Cannot be more than " + std::to_string(MAX_EXCLUSIVESEND_ROUNDS) + " nor less than " + std::to_string(MIN_EXCLUSIVESEND_ROUNDS) +
            "\nExamples:\n"
            + HelpExampleCli("setexclusivesendrounds", "4")
            + HelpExampleRpc("setexclusivesendrounds", "16")
        );

    int nRounds = request.params[0].get_int();

    if (nRounds > MAX_EXCLUSIVESEND_ROUNDS || nRounds < MIN_EXCLUSIVESEND_ROUNDS)
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid number of rounds");

    exclusiveSendClient.nExclusiveSendRounds = nRounds;

    return NullUniValue;
}

UniValue setexclusivesendamount(const JSONRPCRequest& request)
{
    CWallet* const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "setexclusivesendamount amount\n"
            "\nSet the goal amount in " + CURRENCY_UNIT + " for ExclusiveSend mixing.\n"
            "\nArguments:\n"
            "1. amount         (numeric, required) The default amount is " + std::to_string(DEFAULT_EXCLUSIVESEND_AMOUNT) +
            " Cannot be more than " + std::to_string(MAX_EXCLUSIVESEND_AMOUNT) + " nor less than " + std::to_string(MIN_EXCLUSIVESEND_AMOUNT) +
            "\nExamples:\n"
            + HelpExampleCli("setexclusivesendamount", "500")
            + HelpExampleRpc("setexclusivesendamount", "208")
        );

    int nAmount = request.params[0].get_int();

    if (nAmount > MAX_EXCLUSIVESEND_AMOUNT || nAmount < MIN_EXCLUSIVESEND_AMOUNT)
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid amount of " + CURRENCY_UNIT + " as mixing goal amount");

    exclusiveSendClient.nExclusiveSendAmount = nAmount;
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            + (!fLiteMode ?
            "  \"exclusivesend_balance\": xxxxxx, (numeric) the anonymized trivechain balance of the wallet in " + CURRENCY_UNIT + "\n" : "") +
    ...
    if(!fLiteMode)
        obj.push_back(Pair("exclusivesend_balance",       ValueFromAmount(pwallet->GetAnonymizedBalance())));
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of trivechain addresses to filter\n"
            "    [\n"
            "      \"address\"     (string) trivechain address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the trivechain address\n"
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Trivechain address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        entry.push_back(Pair("ps_rounds", pwallet->GetCappedOutpointExclusiveSendRounds(COutPoint(out.tx->GetHash(), out.i))));
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The trivechain address to receive the change\n"
                            ...
                            "                              Those recipients will receive less trivechain than you enter in their corresponding amount field.\n"
    ...
    if (request.params.size() > 1) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid trivechain address");
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "setexclusivesendrounds",     &setexclusivesendrounds,     true,   {"rounds"} },
    { "wallet",             "setexclusivesendamount",     &setexclusivesendamount,     true,   {"amount"} },
    ...
    { "wallet",             "directsendtoaddress",     &directsendtoaddress,     false,  {"address","amount","comment","comment_to","subtractfeefromamount"} },
