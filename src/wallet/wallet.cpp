...
{
    BOOST_FOREACH(CAmount d, CExclusiveSend::GetStandardDenominations())
...
int CWallet::GetOutpointExclusiveSendRounds(const COutPoint& outpoint) const
{
    LOCK(cs_wallet);
    int realExclusiveSendRounds = GetRealOutpointExclusiveSendRounds(outpoint, 0);
    return realExclusiveSendRounds > privateSendClient.nExclusiveSendRounds ? privateSendClient.nExclusiveSendRounds : realExclusiveSendRounds;
}
    ...
    if (!IsCoinBase())
    {
        if (GetDepthInMainChain() == 0 && !isAbandoned() && InMempool()) {
            ...
            if(strCommand == NetMsgType::TXLOCKREQUEST) {
                directsend.ProcessTxLockRequest(((CTxLockRequest)*this), *connman);
            }
            if (connman) {
...
CAmount CWalletTx::GetAnonymizedCredit(bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        const int nRounds = pwallet->GetOutpointExclusiveSendRounds(outpoint);
        if(nRounds >= privateSendClient.nExclusiveSendRounds){
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            if (!MoneyRange(nCredit))
...
float CWallet::GetAverageAnonymizedRounds() const
{
    ...
    for (auto& outpoint : setWalletUTXO) {
        ...
        nTotal += GetOutpointExclusiveSendRounds(outpoint);
        nCount++;
    }
...
CAmount CWallet::GetNormalizedAnonymizedBalance() const
{
    ...
    for (auto& outpoint : setWalletUTXO) {
        ...
        int nRounds = GetOutpointExclusiveSendRounds(outpoint);
        nTotal += it->second.vout[outpoint.n].nValue * nRounds / privateSendClient.nExclusiveSendRounds;
    }
...
CAmount CWallet::GetNeedsToBeAnonymizedBalance(CAmount nMinBalance) const
{
    ...
    CAmount nNeedsToAnonymizeBalance = privateSendClient.nExclusiveSendAmount*COIN - nAnonymizedBalance;
    ...
    if (nNeedsToAnonymizeBalance > CExclusiveSend::GetMaxPoolAmount()) nNeedsToAnonymizeBalance = CExclusiveSend::GetMaxPoolAmount();
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeZeroValue, AvailableCoinsType nCoinType, bool fUseDirectSend) const
{
    vCoins.clear();

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            // do not use IX for inputs that have less then DIRECTSEND_CONFIRMATIONS_REQUIRED blockchain confirmations
            if (fUseDirectSend && nDepth < DIRECTSEND_CONFIRMATIONS_REQUIRED)
                continue;
...
static void ApproximateBestSubset(vector<pair<CAmount, pair<const CWalletTx*,unsigned int> > >vValue, const CAmount& nTotalLower, const CAmount& nTargetValue,
                                  vector<char>& vfBest, CAmount& nBest, int iterations = 1000, bool fUseDirectSend = false)
{
...
bool less_then_denom (const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH(CAmount d, CExclusiveSend::GetStandardDenominations()) // loop through predefined denoms
    {
...
bool CWallet::SelectCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, vector<COutput> vCoins,
                                 set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, bool fUseDirectSend) const
{
    setCoinsRet.clear();
    nValueRet = 0;
...
//            if (fDebug) LogPrint("selectcoins", "value %s confirms %d\n", FormatMoney(pcoin->vout[output.i].nValue), output.nDepth);
    ...
    if (nBest != nTargetValue && nTotalLower >= nTargetValue + MIN_CHANGE)
        ApproximateBestSubset(vValue, nTotalLower, nTargetValue + MIN_CHANGE, vfBest, nBest, fUseDirectSend);
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType nCoinType, bool fUseDirectSend) const
{
    // Note: this function should never be used for "always free" tx types like dstx

    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, nCoinType, fUseDirectSend);
    ...
    if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs)
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if(nCoinType == ONLY_DENOMINATED) {
                ...
                if(nRounds < privateSendClient.nExclusiveSendRounds) continue;
            }
    ...
    if(nCoinType == ONLY_DENOMINATED) {
        ...
        CAmount nSmallestDenom = vecExclusiveSendDenominations.back();
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH(CAmount nDenom, vecExclusiveSendDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
    ...
    bool res = nTargetValue <= nValueFromPresetInputs ||
        SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, 1, 6, vCoins, setCoinsRet, nValueRet, fUseDirectSend) ||
        SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, 1, 1, vCoins, setCoinsRet, nValueRet, fUseDirectSend) ||
        (bSpendZeroConfChange && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, 0, 1, vCoins, setCoinsRet, nValueRet, fUseDirectSend));
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nExclusiveSendRoundsMin, int nExclusiveSendRoundsMax)
{
    vecTxInRet.clear();
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            int nRounds = GetOutpointExclusiveSendRounds(txin.prevout);
            if(nRounds >= nExclusiveSendRoundsMax) continue;
            if(nRounds < nExclusiveSendRoundsMin) continue;

            BOOST_FOREACH(int nBit, vecBits) {
                if(out.tx->vout[out.i].nValue == vecExclusiveSendDenominations[nBit]) {
    ...
    for (auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                // ignore anonymized
                if(GetOutpointExclusiveSendRounds(COutPoint(outpoint.hash, i)) >= privateSendClient.nExclusiveSendRounds) continue;
            }
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nExclusiveSendRoundsMin, int nExclusiveSendRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, nExclusiveSendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT1000IFMN : ONLY_DENOMINATED);
...
bool CWallet::IsCollateralAmount(CAmount nInputAmount) const
{
    ...
    return  nInputAmount >  CExclusiveSend::GetCollateralAmount() &&
            nInputAmount <= CExclusiveSend::GetMaxCollateralAmount() &&
            nInputAmount %  CExclusiveSend::GetCollateralAmount() == 0;
}
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    CTxOut txout = CTxOut(nValue - CExclusiveSend::GetCollateralAmount(), scriptChange);
...
bool CWallet::GetBudgetSystemCollateralTX(CTransaction& tx, uint256 hash, CAmount amount, bool fUseDirectSend)
{
    CWalletTx wtx;
    if(GetBudgetSystemCollateralTX(wtx, hash, amount, fUseDirectSend)){
        tx = (CTransaction)wtx;
        return true;
    }
    return false;
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, CAmount amount, bool fUseDirectSend)
{
    // make our change address
    CReserveKey reservekey(this);
...
bool CWallet::CreateTransaction(const vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet,
                                int& nChangePosRet, std::string& strFailReason, const CCoinControl* coinControl, bool sign, AvailableCoinsType nCoinType, bool fUseDirectSend)
{
    CAmount nFeePay = fUseDirectSend ? CTxLockRequest().GetMinFee() : 0;
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseDirectSend))
                {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 TRVC.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough ExclusiveSend non-denominated funds for this transaction that are not equal 1000 TRVC.");
                        ...
                        if (fUseDirectSend) {
                            ...
                            strFailReason += " " + strprintf(_("DirectSend requires inputs with at least %d confirmations, you might need to wait a few minutes and try again."), DIRECTSEND_CONFIRMATIONS_REQUIRED);
                        }
                    }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-trivecoin-address
                        CScript scriptChange;
...
int CMerkleTx::GetDepthInMainChain(const CBlockIndex* &pindexRet, bool enableIX) const
{
    ...
    if(enableIX && nResult < 6 && directsend.IsLockedDirectSendTransaction(GetHash()))
        return nDirectSendDepth + nResult;
