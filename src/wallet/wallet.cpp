...
{
    for (const auto& d : CExclusiveSend::GetStandardDenominations()) {
...
bool CWallet::AbandonTransaction(const uint256& hashTx)
{
    ...
    if (origtx.GetDepthInMainChain() > 0 || origtx.InMempool() || origtx.IsLockedByDirectSend()) {
        return false;
    }
...
// Recursively determine the rounds of a given input (How deep is the ExclusiveSend chain for a given input)
int CWallet::GetRealOutpointExclusiveSendRounds(const COutPoint& outpoint, int nRounds) const
{
    static std::map<uint256, CMutableTransaction> mDenomWtxes;

    if(nRounds >= MAX_EXCLUSIVESEND_ROUNDS) {
        // there can only be MAX_EXCLUSIVESEND_ROUNDS rounds max
        return MAX_EXCLUSIVESEND_ROUNDS - 1;
    }
    ...
    if(wtx != NULL)
    {
        ...
        if (mdwi == mDenomWtxes.end()) {
            ...
            LogPrint("exclusivesend", "GetRealOutpointExclusiveSendRounds INSERTING %s\n", hash.ToString());
        ...
        if (nout >= wtx->tx->vout.size()) {
            ...
            LogPrint("exclusivesend", "GetRealOutpointExclusiveSendRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }

        if (CExclusiveSend::IsCollateralAmount(wtx->tx->vout[nout].nValue)) {
        ...
        if (!CExclusiveSend::IsDenominatedAmount(wtx->tx->vout[nout].nValue)) { //NOT DENOM
        ...
        for (const auto& out : wtx->tx->vout) {
            fAllDenoms = fAllDenoms && CExclusiveSend::IsDenominatedAmount(out.nValue);
        }
        ...
        for (const auto& txinNext : wtx->tx->vin) {
            if (IsMine(txinNext)) {
                int n = GetRealOutpointExclusiveSendRounds(txinNext.prevout, nRounds + 1);
        ...
        mDenomWtxes[hash].vout[nout].nRounds = fDenomFound
                ? (nShortest >= MAX_EXCLUSIVESEND_ROUNDS - 1 ? MAX_EXCLUSIVESEND_ROUNDS : nShortest + 1) // good, we a +1 to the shortest one but only MAX_EXCLUSIVESEND_ROUNDS rounds max allowed
...
int CWallet::GetCappedOutpointExclusiveSendRounds(const COutPoint& outpoint) const
{
    LOCK(cs_wallet);
    int realExclusiveSendRounds = GetRealOutpointExclusiveSendRounds(outpoint);
    return realExclusiveSendRounds > exclusiveSendClient.nExclusiveSendRounds ? exclusiveSendClient.nExclusiveSendRounds : realExclusiveSendRounds;
}
...
bool CWallet::IsDenominated(const COutPoint& outpoint) const
{
    ...
    return CExclusiveSend::IsDenominatedAmount(it->second.tx->vout[outpoint.n].nValue);
}
...
void CWallet::ReacceptWalletTransactions()
{
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
    {
        ...
        if (!wtx.IsCoinBase() && (nDepth == 0 && !wtx.IsLockedByDirectSend() && !wtx.isAbandoned())) {
    ...
    if (!IsCoinBase() && !isAbandoned() && GetDepthInMainChain() == 0)
    {
        ...
        if (InMempool() || AcceptToMemoryPool(maxTxFee, state)) {
            ...
            if ((strCommand == NetMsgType::TXLOCKREQUEST) ||
                ((CTxLockRequest(*this).IsSimple()) && CDirectSend::CanAutoLock())) {
                if (directsend.ProcessTxLockRequest((CTxLockRequest)*this, *connman)) {
                    directsend.AcceptLockRequest((CTxLockRequest)*this);
                } else {
                    directsend.RejectLockRequest((CTxLockRequest)*this);
                }
            }
...
CAmount CWalletTx::GetAnonymizedCredit(bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < tx->vout.size(); i++)
    {
        ...
        if (pwallet->IsSpent(hashTx, i) || !CExclusiveSend::IsDenominatedAmount(txout.nValue)) continue;

        const int nRounds = pwallet->GetCappedOutpointExclusiveSendRounds(outpoint);
        if (nRounds >= exclusiveSendClient.nExclusiveSendRounds){
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            if (!MoneyRange(nCredit))
                throw std::runtime_error(std::string(__func__) + ": value out of range");
        }
...
CAmount CWallet::GetAnonymizableBalance(bool fSkipDenominated, bool fSkipUnconfirmed) const
{
    ...
    const CAmount nSmallestDenom = CExclusiveSend::GetSmallestDenomination();
    const CAmount nMixingCollateral = CExclusiveSend::GetCollateralAmount();
    for (const auto& item : vecTally) {
        bool fIsDenominated = CExclusiveSend::IsDenominatedAmount(item.nAmount);
...
float CWallet::GetAverageAnonymizedRounds() const
{
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        nTotal += GetCappedOutpointExclusiveSendRounds(outpoint);
        nCount++;
    }
...
CAmount CWallet::GetNormalizedAnonymizedBalance() const
{
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        if (!CExclusiveSend::IsDenominatedAmount(nValue)) continue;
        if (it->second.GetDepthInMainChain() < 0) continue;

        int nRounds = GetCappedOutpointExclusiveSendRounds(outpoint);
        nTotal += nValue * nRounds / exclusiveSendClient.nExclusiveSendRounds;
    }
...
CAmount CWallet::GetUnconfirmedBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            if (!pcoin->IsTrusted() && pcoin->GetDepthInMainChain() == 0 && !pcoin->IsLockedByDirectSend() && pcoin->InMempool())
                nTotal += pcoin->GetAvailableCredit();
        }
    }
    return nTotal;
...
void CWallet::AvailableCoins(std::vector<COutput>& vCoins, bool fOnlySafe, const CCoinControl *coinControl, bool fIncludeZeroValue, AvailableCoinsType nCoinType, bool fUseDirectSend) const
{
    vCoins.clear();

    {
        LOCK2(cs_main, cs_wallet);
        int nDirectSendConfirmationsRequired = Params().GetConsensus().nDirectSendConfirmationsRequired;
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            // do not use IX for inputs that have less then nDirectSendConfirmationsRequired blockchain confirmations
            if (fUseDirectSend && nDepth < nDirectSendConfirmationsRequired)
                continue;
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    found = CExclusiveSend::IsDenominatedAmount(pcoin->tx->vout[i].nValue);
                } else if(nCoinType == ONLY_NONDENOMINATED) {
                    if (CExclusiveSend::IsCollateralAmount(pcoin->tx->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !CExclusiveSend::IsDenominatedAmount(pcoin->tx->vout[i].nValue);
                } else if(nCoinType == ONLY_1000) {
                    found = pcoin->tx->vout[i].nValue == 1000*COIN;
                } else if(nCoinType == ONLY_EXCLUSIVESEND_COLLATERAL) {
                    found = CExclusiveSend::IsCollateralAmount(pcoin->tx->vout[i].nValue);
                } else {
                    found = true;
                }
                if(!found) continue;
...
static void ApproximateBestSubset(std::vector<std::pair<CAmount, std::pair<const CWalletTx*,unsigned int> > >vValue, const CAmount& nTotalLower, const CAmount& nTargetValue,
                                  std::vector<char>& vfBest, CAmount& nBest, bool fUseDirectSend = false, int iterations = 1000)
{
    ...
    if (!llmq::IsOldDirectSendEnabled()) {
        // The new system does not require special handling for DirectSend as this is all done in CDirectSendManager.
        // There is also no need for an extra fee anymore.
        fUseDirectSend = false;
    }
    ...
    for (int nRep = 0; nRep < iterations && nBest != nTargetValue; nRep++)
    {
        ...
        for (int nPass = 0; nPass < 2 && !fReachedTarget; nPass++)
        {
            ...
            {
                if (fUseDirectSend && nTotal + vValue[i].first > sporkManager.GetSporkValue(SPORK_5_DIRECTSEND_MAX_VALUE)*COIN) {
                    continue;
                }
...
bool less_then_denom (const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    for (const auto& d : CExclusiveSend::GetStandardDenominations()) // loop through predefined denoms
    {
...
bool CWallet::SelectCoinsMinConf(const CAmount& nTargetValue, const int nConfMine, const int nConfTheirs, const uint64_t nMaxAncestors, std::vector<COutput> vCoins,
                                 std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, AvailableCoinsType nCoinType, bool fUseDirectSend) const
{
    setCoinsRet.clear();
    nValueRet = 0;
    ...
    coinLowestLarger.first = fUseDirectSend
                                        ? sporkManager.GetSporkValue(SPORK_5_DIRECTSEND_MAX_VALUE)*COIN
    ...
    if (nCoinType == ONLY_DENOMINATED) {
        ...
        // try not to use denominated coins when not needed, save denoms for exclusivesend
...
//            if (fDebug) LogPrint("selectcoins", "value %s confirms %d\n", FormatMoney(pcoin->vout[output.i].nValue), output.nDepth);
            ...
            if (tryDenom == 0 && CExclusiveSend::IsDenominatedAmount(n)) continue; // we don't want denom values on first run

            if (nCoinType == ONLY_DENOMINATED) {
                // Make sure it's actually anonymized
                COutPoint outpoint = COutPoint(pcoin->GetHash(), i);
                int nRounds = GetRealOutpointExclusiveSendRounds(outpoint);
                if (nRounds < exclusiveSendClient.nExclusiveSendRounds) continue;
            }
    ...
    ApproximateBestSubset(vValue, nTotalLower, nTargetValue, vfBest, nBest, fUseDirectSend);
    if (nBest != nTargetValue && nTotalLower >= nTargetValue + nMinChange)
        ApproximateBestSubset(vValue, nTotalLower, nTargetValue + nMinChange, vfBest, nBest, fUseDirectSend);
...
bool CWallet::SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType nCoinType, bool fUseDirectSend) const
{
    ...
    bool res = nTargetValue <= nValueFromPresetInputs ||
        SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, 1, 6, 0, vCoins, setCoinsRet, nValueRet, nCoinType, fUseDirectSend) ||
        SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, 1, 1, 0, vCoins, setCoinsRet, nValueRet, nCoinType, fUseDirectSend) ||
        (bSpendZeroConfChange && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, 0, 1, 2, vCoins, setCoinsRet, nValueRet, nCoinType, fUseDirectSend)) ||
        (bSpendZeroConfChange && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, 0, 1, std::min((size_t)4, nMaxChainLength/3), vCoins, setCoinsRet, nValueRet, nCoinType, fUseDirectSend)) ||
        (bSpendZeroConfChange && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, 0, 1, nMaxChainLength/2, vCoins, setCoinsRet, nValueRet, nCoinType, fUseDirectSend)) ||
        (bSpendZeroConfChange && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, 0, 1, nMaxChainLength, vCoins, setCoinsRet, nValueRet, nCoinType, fUseDirectSend)) ||
        (bSpendZeroConfChange && !fRejectLongChains && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, 0, 1, std::numeric_limits<uint64_t>::max(), vCoins, setCoinsRet, nValueRet, nCoinType, fUseDirectSend));
...
bool CWallet::SelectPSInOutPairsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector< std::pair<CTxDSIn, CTxOut> >& vecPSInOutPairsRet)
{
    ...
    std::vector<CAmount> vecExclusiveSendDenominations = CExclusiveSend::GetStandardDenominations();
    for (const auto& out : vCoins) {
        ...
        int nRounds = GetRealOutpointExclusiveSendRounds(txin.prevout);
        if (nRounds >= exclusiveSendClient.nExclusiveSendRounds) continue;

        for (const auto& nBit : vecBits) {
            if (nValue != vecExclusiveSendDenominations[nBit]) continue;
            nValueTotal += nValue;
            ...
            nDenomResult |= 1 << nBit;
            LogPrint("exclusivesend", "CWallet::%s -- hash: %s, nValue: %d.%08d, nRounds: %d\n",
...
bool CWallet::SelectCoinsGroupedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable, bool fSkipUnconfirmed, int nMaxOupointsPerAddress) const
{
    ...
    CAmount nSmallestDenom = CExclusiveSend::GetSmallestDenomination();
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fSkipDenominated && CExclusiveSend::IsDenominatedAmount(wtx.tx->vout[i].nValue)) continue;

            if(fAnonymizable) {
                // ignore collaterals
                if(CExclusiveSend::IsCollateralAmount(wtx.tx->vout[i].nValue)) continue;
                if(fMasternodeMode && wtx.tx->vout[i].nValue == 1000*COIN) continue;
                // ignore outputs that are 10 times smaller then the smallest denomination
                // otherwise they will just lead to higher fee / lower priority
                if(wtx.tx->vout[i].nValue <= nSmallestDenom/10) continue;
                // ignore anonymized
                if(GetCappedOutpointExclusiveSendRounds(COutPoint(outpoint.hash, i)) >= exclusiveSendClient.nExclusiveSendRounds) continue;
            }
...
bool CWallet::SelectPrivateCoins(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nExclusiveSendRoundsMin, int nExclusiveSendRoundsMax) const
{
    ...
    std::vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, nExclusiveSendRoundsMin < 0 ? ONLY_NONDENOMINATED : ONLY_DENOMINATED);
    ...
    for (const auto& out : vCoins)
    {
        ...
        if(CExclusiveSend::IsCollateralAmount(out.tx->tx->vout[out.i].nValue)) continue;
        ...
        if(nValueRet + out.tx->tx->vout[out.i].nValue <= nValueMax){
            ...
            int nRounds = GetCappedOutpointExclusiveSendRounds(txin.prevout);
            if(nRounds > nExclusiveSendRoundsMax) continue;
            if(nRounds < nExclusiveSendRoundsMin) continue;
...
bool CWallet::GetCollateralTxDSIn(CTxDSIn& txdsinRet, CAmount& nValueRet) const
{
    ...
    AvailableCoins(vCoins, true, NULL, false, ONLY_EXCLUSIVESEND_COLLATERAL);
...
{
    std::vector<COutput> vCoins;
    AvailableCoins(vCoins, fOnlyConfirmed, NULL, false, ONLY_EXCLUSIVESEND_COLLATERAL);
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    if (!GetCollateralTxDSIn(txdsinCollateral, nValue)) {
        strReason = "ExclusiveSend requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    // CExclusiveSend::IsCollateralAmount in GetCollateralTxDSIn should already take care of this
    if (nValue >= CExclusiveSend::GetCollateralAmount() * 2) {
        ...
        reservekey.KeepKey();
        // return change
        txCollateral.vout.push_back(CTxOut(nValue - CExclusiveSend::GetCollateralAmount(), scriptChange));
    } else { // nValue < CExclusiveSend::GetCollateralAmount() * 2
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, CAmount amount, bool fUseDirectSend, const COutPoint& outpoint)
{
    // make our change address
    CReserveKey reservekey(this);
    ...
    bool success = CreateTransaction(vecSend, tx, reservekey, nFeeRet, nChangePosRet, strFail, &coinControl, true, ALL_COINS, fUseDirectSend);
    if(!success){
...
bool CWallet::CreateTransaction(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet,
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl* coinControl, bool sign, AvailableCoinsType nCoinType, bool fUseDirectSend, int nExtraPayloadSize)
{
    if (!llmq::IsOldDirectSendEnabled()) {
        // The new system does not require special handling for DirectSend as this is all done in CDirectSendManager.
        // There is also no need for an extra fee anymore.
    ...
    CAmount nFeePay = fUseDirectSend ? CTxLockRequest().GetMinFee(true) : 0;
...

    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            AvailableCoins(vAvailableCoins, true, coinControl, false, nCoinType, fUseDirectSend);
            int nDirectSendConfirmationsRequired = Params().GetConsensus().nDirectSendConfirmationsRequired;
            ...
            while (true)
            {
                ...
                CAmount nValueIn = 0;
                setCoins.clear();
                if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseDirectSend))
                {
                    if (nCoinType == ONLY_NONDENOMINATED) {
                        strFailReason = _("Unable to locate enough ExclusiveSend non-denominated funds for this transaction.");
                    } else if (nCoinType == ONLY_DENOMINATED) {
                        strFailReason = _("Unable to locate enough ExclusiveSend denominated funds for this transaction.");
                        strFailReason += " " + _("ExclusiveSend uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                    } else if (nValueIn < nValueToSelect) {
                        strFailReason = _("Insufficient funds.");
                        if (fUseDirectSend) {
                            // could be not true but most likely that's the reason
                            strFailReason += " " + strprintf(_("DirectSend requires inputs with at least %d confirmations, you might need to wait a few minutes and try again."), nDirectSendConfirmationsRequired);
                        }
                    }
...
{
    {
        ...
        LogPrintf("CommitTransaction:\n%s", wtxNew.tx->ToString());
        {
            ...
            BOOST_FOREACH(const CTxIn& txin, wtxNew.tx->vin)
            {
                ...
                exclusiveSendClient.RemoveSkippedDenom(coin.tx->vout[txin.prevout.n].nValue);
            }
        }
...
std::map<CTxDestination, CAmount> CWallet::GetAddressBalances()
{
...

    {
        ...
        BOOST_FOREACH(PAIRTYPE(uint256, CWalletTx) walletEntry, mapWallet)
        {
            ...
            int nDepth = pcoin->GetDepthInMainChain();
            if ((nDepth < (pcoin->IsFromMe(ISMINE_ALL) ? 0 : 1)) && !pcoin->IsLockedByDirectSend())
                continue;
...
CAmount CWallet::GetAccountBalance(CWalletDB& walletdb, const std::string& strAccount, int nMinDepth, const isminefilter& filter, bool fAddLocked)
{
    ...
    for (std::map<uint256, CWalletTx>::iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
    {
        ...
        if (nReceived != 0 && ((wtx.GetDepthInMainChain() >= nMinDepth) || (fAddLocked && wtx.IsLockedByDirectSend())))
            nBalance += nReceived;
        nBalance -= nSent + nFee;
    }
...
bool CMerkleTx::IsLockedByDirectSend() const
{
    return directsend.IsLockedDirectSendTransaction(GetHash()) || llmq::quorumDirectSendManager->IsLocked(GetHash());
}

bool CMerkleTx::IsLockedByLLMQDirectSend() const
{
    return llmq::quorumDirectSendManager->IsLocked(GetHash());
}
