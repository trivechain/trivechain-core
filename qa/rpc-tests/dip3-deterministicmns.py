...
class DIP3Test(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("controller node has {} trivechain".format(self.nodes[0].getbalance()))
        ...
        self.test_directsend(10, 3, timeout=20)
    ...
    def test_directsend(self, tx_count, repeat, timeout=20):
        ...
        for j in range(repeat):
            for i in range(tx_count):
                ...
                txid = from_node.directsendtoaddress(to_address, 0.1)
                for node in self.nodes:
                    if node is not None:
