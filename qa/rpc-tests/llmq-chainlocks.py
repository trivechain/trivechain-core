...
class LLMQChainLocksTest(TrivechainTestFramework):
    def __init__(self):
    ...
    def run_test(self):
        ...
        # Enable LLMQ bases DirectSend, which also enables checks for "safe" transactions
        ...
        sleep(1)
        assert(not self.nodes[0].getblock(self.nodes[0].getbestblockhash())["chainlock"])
        # Disable LLMQ based DirectSend for a very short time (this never gets propagated to other nodes)
        self.nodes[0].spork("SPORK_20_DIRECTSEND_LLMQ_BASED", 4070908800)
