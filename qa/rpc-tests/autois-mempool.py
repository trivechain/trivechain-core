...
from test_framework.test_framework import TrivechainTestFramework
...
Checks if automatic DirectSend locks stop working when transaction mempool 
...
class AutoISMempoolTest(TrivechainTestFramework):
    def __init__(self):
    ...
    def test_auto(self, new_is = False):
        ...
        self.nodes[0].spork("SPORK_2_DIRECTSEND_ENABLED", 4070908800)
        self.wait_for_sporks_same()
        self.fill_mempool()
        self.set_autois_spork_state(self.nodes[0], True)
        self.nodes[0].spork("SPORK_2_DIRECTSEND_ENABLED", 0)
        self.wait_for_sporks_same()
        ...
        assert(not self.send_regular_directsend(sender, receiver, False) if new_is else self.send_regular_directsend(sender, receiver))
