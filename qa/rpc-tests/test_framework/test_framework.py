...
class TrivechainTestFramework(BitcoinTestFramework):
    ...
    def get_autois_spork_state(self, node):
        ...
        return info['SPORK_16_DIRECTSEND_AUTOLOCKS']
    ...
    def set_autois_spork_state(self, node, state):
        ...
        if state:
            ...
            value = 4070908800
        node.spork('SPORK_16_DIRECTSEND_AUTOLOCKS', value)
    ...
    # sends regular directsend with high fee
    def send_regular_directsend(self, sender, receiver, check_fee = True):
        receiver_addr = receiver.getnewaddress()
        txid = sender.directsendtoaddress(receiver_addr, 1.0)
        if (check_fee):
